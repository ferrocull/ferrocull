...
**Burst**:
A run of **3 or more** consecutive media files whose capture times are each within 1 second of the previous one; two shots within a second do not form a burst. A collapsed burst behaves as one virtual photo across the whole display sequence, shown as its [[representative]], and rating/labelling/tagging any member applies to all members.
